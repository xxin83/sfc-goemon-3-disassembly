...
def patch_slots(rom: bytes, assignments: dict[int, int]) -> bytes:
    ...
    for code, tile in assignments.items():
        ...
        if code in RESERVED or code not in available:
...
def main() -> None:
    ...
    args.output.write_bytes(patch_slots(args.rom.read_bytes(), assignments))
